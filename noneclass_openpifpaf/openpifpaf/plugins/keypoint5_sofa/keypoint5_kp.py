...
class Keypoint5Kp(DataModule):
    ...
    train_annotations = 'data-keypoint-5/annotations_sofa/keypoint5_train.json'
    val_annotations = 'data-keypoint-5/annotations_sofa/keypoint5_test.json'
    eval_annotations = val_annotations
    train_image_dir = 'data-keypoint-5/images_sofa/train/'
    val_image_dir = 'data-keypoint-5/images_sofa/test/'
    ...
    def __init__(self):
        ...
        if self.weights is not None:
            ...
            caf_weights = None
        cif = headmeta.Cif('cif', 'keypoint5_sofa',
        ...
        caf = headmeta.Caf('caf', 'keypoint5_sofa',
                           keypoints=self.FURNITURE_KEYPOINTS,
                           sigmas=self.FURNITURE_SIGMAS,
                           pose=self.FURNITURE_POSE,
    ...
    def cli(cls, parser: argparse.ArgumentParser):
        group = parser.add_argument_group('data module keypoint5_sofa')

        group.add_argument('--keypoint5_sofa-train-annotations',
                           default=cls.train_annotations)
        group.add_argument('--keypoint5_sofa-val-annotations',
                           default=cls.val_annotations)
        group.add_argument('--keypoint5_sofa-train-image-dir',
                           default=cls.train_image_dir)
        group.add_argument('--keypoint5_sofa-val-image-dir',
                           default=cls.val_image_dir)

        group.add_argument('--keypoint5_sofa-square-edge',
                           default=cls.square_edge, type=int,
                           help='square edge of input images')
        assert not cls.extended_scale
        group.add_argument('--keypoint5_sofa-extended-scale',
                           default=False, action='store_true',
                           help='augment with an extended scale range')
        group.add_argument('--keypoint5_sofa-orientation-invariant',
                           default=cls.orientation_invariant, type=float,
                           help='augment with random orientations')
        group.add_argument('--keypoint5_sofa-blur',
                           default=cls.blur, type=float,
                           help='augment with blur')
        assert cls.augmentation
        group.add_argument('--keypoint5_sofa-no-augmentation',
                           dest='keypoint5_sofa_augmentation',
                           default=True, action='store_false',
                           help='do not apply data augmentation')
        group.add_argument('--keypoint5_sofa-rescale-images',
                           default=cls.rescale_images, type=float,
                           help='overall rescale factor for images')
        group.add_argument('--keypoint5_sofa-upsample',
                           default=cls.upsample_stride, type=int,
                           help='head upsample stride')
        group.add_argument('--keypoint5_sofa-min-kp-anns',
                           default=cls.min_kp_anns, type=int,
                           help='filter images with fewer keypoint annotations')
        group.add_argument('--keypoint5_sofa-bmin',
                           default=cls.b_min, type=int,
                           help='b minimum in pixels')
        group.add_argument('--keypoint5_sofa-apply-local-centrality-weights',
                           dest='keypoint5_sofa_apply_local_centrality',
                           default=False, action='store_true',
                           help='Weigh the CIF and CAF head during training.')

        # evaluation
        assert cls.eval_annotation_filter
        group.add_argument('--keypoint5_sofa-no-eval-annotation-filter',
                           dest='keypoint5_sofa_eval_annotation_filter',
                           default=True, action='store_false')
        group.add_argument('--keypoint5_sofa-eval-long-edge', default=cls.eval_long_edge, type=int,
                           help='set to zero to deactivate rescaling')
        assert not cls.eval_extended_scale
        group.add_argument('--keypoint5_sofa-eval-extended-scale', default=False, action='store_true')
        group.add_argument('--keypoint5_sofa-eval-orientation-invariant',
                           default=cls.eval_orientation_invariant, type=float)
        group.add_argument('--keypoint5_sofa-use-10-kps', default=False, action='store_true',
                           help=(' '))
    ...
    def configure(cls, args: argparse.Namespace):
        ...
        # keypoint5_sofa specific
        cls.train_annotations = args.keypoint5_sofa_train_annotations
        cls.val_annotations = args.keypoint5_sofa_val_annotations
        cls.eval_annotations = cls.val_annotations
        cls.train_image_dir = args.keypoint5_sofa_train_image_dir
        cls.val_image_dir = args.keypoint5_sofa_val_image_dir
        cls.eval_image_dir = cls.val_image_dir

        cls.square_edge = args.keypoint5_sofa_square_edge
        cls.extended_scale = args.keypoint5_sofa_extended_scale
        cls.orientation_invariant = args.keypoint5_sofa_orientation_invariant
        cls.blur = args.keypoint5_sofa_blur
        cls.augmentation = args.keypoint5_sofa_augmentation  # loaded by the dest name
        cls.rescale_images = args.keypoint5_sofa_rescale_images
        cls.upsample_stride = args.keypoint5_sofa_upsample
        cls.min_kp_anns = args.keypoint5_sofa_min_kp_anns
        cls.b_min = args.keypoint5_sofa_bmin
        if True:#args.keypoint5_sofa_use_10_kps:
        ...
        # evaluation
        cls.eval_annotation_filter = args.keypoint5_sofa_eval_annotation_filter
        cls.eval_long_edge = args.keypoint5_sofa_eval_long_edge
        cls.eval_orientation_invariant = args.keypoint5_sofa_eval_orientation_invariant
        cls.eval_extended_scale = args.keypoint5_sofa_eval_extended_scale
        
        if args.keypoint5_sofa_apply_local_centrality:
            '''
            if args.keypoint5_sofa_use_10_kps:
