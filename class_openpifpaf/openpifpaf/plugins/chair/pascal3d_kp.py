...
class Pascal3dKp(DataModule):
    ...
    def cli(cls, parser: argparse.ArgumentParser):
        group = parser.add_argument_group('data module chair')

        group.add_argument('--chair-train-annotations',
                           default=cls.train_annotations)
        group.add_argument('--chair-val-annotations',
                           default=cls.val_annotations)
        group.add_argument('--chair-train-image-dir',
                           default=cls.train_image_dir)
        group.add_argument('--chair-val-image-dir',
                           default=cls.val_image_dir)

        group.add_argument('--chair-square-edge',
                           default=cls.square_edge, type=int,
                           help='square edge of input images')
        assert not cls.extended_scale
        group.add_argument('--chair-extended-scale',
                           default=False, action='store_true',
                           help='augment with an extended scale range')
        group.add_argument('--chair-orientation-invariant',
                           default=cls.orientation_invariant, type=float,
                           help='augment with random orientations')
        group.add_argument('--chair-blur',
                           default=cls.blur, type=float,
                           help='augment with blur')
        assert cls.augmentation
        group.add_argument('--chair-no-augmentation',
                           dest='chair_augmentation',
                           default=True, action='store_false',
                           help='do not apply data augmentation')
        group.add_argument('--chair-rescale-images',
                           default=cls.rescale_images, type=float,
                           help='overall rescale factor for images')
        group.add_argument('--chair-upsample',
                           default=cls.upsample_stride, type=int,
                           help='head upsample stride')
        group.add_argument('--chair-min-kp-anns',
                           default=cls.min_kp_anns, type=int,
                           help='filter images with fewer keypoint annotations')
        group.add_argument('--chair-bmin',
                           default=cls.b_min, type=int,
                           help='b minimum in pixels')
        group.add_argument('--chair-apply-local-centrality-weights',
                           dest='chair_apply_local_centrality',
                           default=False, action='store_true',
        ...
        group.add_argument('--chair-no-eval-annotation-filter',
                           dest='chair_eval_annotation_filter',
                           default=True, action='store_false')
        group.add_argument('--chair-eval-long-edge', default=cls.eval_long_edge, type=int,
                           help='set to zero to deactivate rescaling')
        assert not cls.eval_extended_scale
        group.add_argument('--chair-eval-extended-scale', default=False, action='store_true')
        group.add_argument('--chair-eval-orientation-invariant',
                           default=cls.eval_orientation_invariant, type=float)
        group.add_argument('--chair-use-10-kps', default=False, action='store_true',
                           help=(' '))
    ...
    def configure(cls, args: argparse.Namespace):
        ...
        # chair specific
        cls.train_annotations = args.chair_train_annotations
        cls.val_annotations = args.chair_val_annotations
        cls.eval_annotations = cls.val_annotations
        cls.train_image_dir = args.chair_train_image_dir
        cls.val_image_dir = args.chair_val_image_dir
        cls.eval_image_dir = cls.val_image_dir

        cls.square_edge = args.chair_square_edge
        cls.extended_scale = args.chair_extended_scale
        cls.orientation_invariant = args.chair_orientation_invariant
        cls.blur = args.chair_blur
        cls.augmentation = args.chair_augmentation  # loaded by the dest name
        cls.rescale_images = args.chair_rescale_images
        cls.upsample_stride = args.chair_upsample
        cls.min_kp_anns = args.chair_min_kp_anns
        cls.b_min = args.chair_bmin
        ...
        # evaluation
        cls.eval_annotation_filter = args.chair_eval_annotation_filter
        cls.eval_long_edge = args.chair_eval_long_edge
        cls.eval_orientation_invariant = args.chair_eval_orientation_invariant
        cls.eval_extended_scale = args.chair_eval_extended_scale

        if args.chair_apply_local_centrality:
            '''
            if args.chair_use_10_kps:
