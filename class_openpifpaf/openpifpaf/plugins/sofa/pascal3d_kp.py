...
class Pascal3dKp(DataModule):
    ...
    train_annotations = 'data-pascal3d/annotations_sofa/pascal3d_train.json'
    val_annotations = 'data-pascal3d/annotations_sofa/pascal3d_val.json'
    eval_annotations = val_annotations ##############################################     important
    train_image_dir = 'data-pascal3d/images_sofa/train/'
    val_image_dir = 'data-pascal3d/images_sofa/val/'
    ...
    def cli(cls, parser: argparse.ArgumentParser):
        group = parser.add_argument_group('data module sofa')

        group.add_argument('--sofa-train-annotations',
                           default=cls.train_annotations)
        group.add_argument('--sofa-val-annotations',
                           default=cls.val_annotations)
        group.add_argument('--sofa-train-image-dir',
                           default=cls.train_image_dir)
        group.add_argument('--sofa-val-image-dir',
                           default=cls.val_image_dir)

        group.add_argument('--sofa-square-edge',
                           default=cls.square_edge, type=int,
                           help='square edge of input images')
        assert not cls.extended_scale
        group.add_argument('--sofa-extended-scale',
                           default=False, action='store_true',
                           help='augment with an extended scale range')
        group.add_argument('--sofa-orientation-invariant',
                           default=cls.orientation_invariant, type=float,
                           help='augment with random orientations')
        group.add_argument('--sofa-blur',
                           default=cls.blur, type=float,
                           help='augment with blur')
        assert cls.augmentation
        group.add_argument('--sofa-no-augmentation',
                           dest='sofa_augmentation',
                           default=True, action='store_false',
                           help='do not apply data augmentation')
        group.add_argument('--sofa-rescale-images',
                           default=cls.rescale_images, type=float,
                           help='overall rescale factor for images')
        group.add_argument('--sofa-upsample',
                           default=cls.upsample_stride, type=int,
                           help='head upsample stride')
        group.add_argument('--sofa-min-kp-anns',
                           default=cls.min_kp_anns, type=int,
                           help='filter images with fewer keypoint annotations')
        group.add_argument('--sofa-bmin',
                           default=cls.b_min, type=int,
                           help='b minimum in pixels')
        group.add_argument('--sofa-apply-local-centrality-weights',
                           dest='sofa_apply_local_centrality',
                           default=False, action='store_true',
        ...
        group.add_argument('--sofa-no-eval-annotation-filter',
                           dest='sofa_eval_annotation_filter',
                           default=True, action='store_false')
        group.add_argument('--sofa-eval-long-edge', default=cls.eval_long_edge, type=int,
                           help='set to zero to deactivate rescaling')
        assert not cls.eval_extended_scale
        group.add_argument('--sofa-eval-extended-scale', default=False, action='store_true')
        group.add_argument('--sofa-eval-orientation-invariant',
                           default=cls.eval_orientation_invariant, type=float)
        group.add_argument('--sofa-use-10-kps', default=False, action='store_true',
                           help=(' '))
    ...
    def configure(cls, args: argparse.Namespace):
        ...
        # sofa specific
        cls.train_annotations = args.sofa_train_annotations
        cls.val_annotations = args.sofa_val_annotations
        cls.eval_annotations = cls.val_annotations
        cls.train_image_dir = args.sofa_train_image_dir
        cls.val_image_dir = args.sofa_val_image_dir
        cls.eval_image_dir = cls.val_image_dir

        cls.square_edge = args.sofa_square_edge
        cls.extended_scale = args.sofa_extended_scale
        cls.orientation_invariant = args.sofa_orientation_invariant
        cls.blur = args.sofa_blur
        cls.augmentation = args.sofa_augmentation  # loaded by the dest name
        cls.rescale_images = args.sofa_rescale_images
        cls.upsample_stride = args.sofa_upsample
        cls.min_kp_anns = args.sofa_min_kp_anns
        cls.b_min = args.sofa_bmin
        ...
        # evaluation
        cls.eval_annotation_filter = args.sofa_eval_annotation_filter
        cls.eval_long_edge = args.sofa_eval_long_edge
        cls.eval_orientation_invariant = args.sofa_eval_orientation_invariant
        cls.eval_extended_scale = args.sofa_eval_extended_scale

        if args.sofa_apply_local_centrality:
            '''
            if args.sofa_use_10_kps:
