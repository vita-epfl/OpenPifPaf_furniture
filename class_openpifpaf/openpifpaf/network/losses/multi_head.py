...
##########################################################################################
        ...
        #                    for ll in l(f, t)]
##############  Original code can not run training for multiple furniture dataset, I do not know why  ########
        flat_head_losses = []
        for l, f, t in zip(self.losses, head_fields, head_targets):
            for ll in l(f, t):
                flat_head_losses.append(ll)
            if ll == None:
                flat_head_losses.append(ll)  ## I printed the headloss and found that one more 'None' is always needed
##############################################################################################################
